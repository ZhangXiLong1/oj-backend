...
(
    id BIGINT auto_increment comment 'id' PRIMARY KEY,
    titile VARCHAR(512) null comment '标题',
    content text null comment '内容',
    tags VARCHAR(1024) null comment '标签列表(json数组)',
    answer text null comment '题目答案',
    submitNum int default 0 not null comment '题目提交数',
    accpetNum int default 0 not null comment '题目通过数',
...
-- 题目提交表
create table if not exists questionSubmit
(
    id         bigint auto_increment comment 'id' primary key,
    language varchar(128) not null comment '编程语言',
    code text not null comment '用户代码',
    judgeInfo text null comment '判题信息(json数组)',
    status int default 0 comment '判题状态(0-未判题 1-判题中 2-成功 3-失败)',
